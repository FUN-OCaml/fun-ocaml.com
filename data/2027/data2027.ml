...
module People = struct
  ...
  let of_metadata (m : metadata) : t =
    {
      slug = m.slug;
      img_src = "/2027/avatars/" ^ m.slug ^ ".jpg";
      name = m.name;
  ...
  let all =
    Read_yaml.yaml_file decode "2027/people.yml"
...
module Sessions = struct
  ...
  let url slug = "/2027/" ^ slug ^ "/"

  let all =
    Read_yaml.yaml_file decode "2027/sessions.yml"
    |> Read_yaml.Result.get_ok ~error:(fun (`Msg m) -> Invalid_argument m)
end
...
module Schedule = struct
  ...
  let day_one =
    Read_yaml.yaml_file schedule_of_yaml "2027/schedule/2027-01-25.yaml"
    |> Read_yaml.Result.get_ok ~error:(fun (`Msg m) -> Invalid_argument m)

  let day_two =
    Read_yaml.yaml_file schedule_of_yaml "2027/schedule/2027-01-26.yaml"
    |> Read_yaml.Result.get_ok ~error:(fun (`Msg m) -> Invalid_argument m)
end
